...
class CoordinateFrame(object):
    ...
    @property
    def pos(self):
        return [self.variables[p] if isinstance(p, str) else p for p in self.__pos]
    @pos.setter
    def pos(self, pos):
        self.__pos=pos
    ...
    @property
    def euler(self):
        return [self.variables[p] if isinstance(p, str) else p for p in self.__euler]
    @euler.setter
...
class OblateEllipsoidFrame(CoordinateFrame):
    ...
    def getLocalFrameRotation(self, natural_location):
        return Rotation.fromAngle(-np.pi/2, 'y')*Rotation.fromAngle(deg2rad(natural_location[1]),'x')*Rotation.fromAngle(-deg2rad(natural_location[0]),'y')
